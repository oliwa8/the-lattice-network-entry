...
function Card({ title, description, icon, delay = 0 }: CardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative p-8 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 cursor-pointer transition-all duration-300"
      style={{ transformStyle: 'preserve-3d' }}
    >
      {/* Holographic effect */}
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-indigo-500/20 via-purple-500/20 to-pink-500/20 opacity-0 hover:opacity-100 transition-opacity duration-300" />
